...
# Define your base path for accessing CSV files
# or leave it as an empty string if you want to use the current directory
# Adjust the path according to your local setup
PATH = '/Users/jessetownsend/Documents/Udacity/datascience_withpython/Mod4-Git/udacity-bikeshare/'
...
def city_filter(PATH):
    ...
    try: 
        ...
        loaded_city = CITY_NAMES[user_city]
        print(f'--- Imported {loaded_city} City Data ---')
        return df
...
def station_stats(no_filter_df):
    ...
    popular_combo = no_filter_df['Station Combo'].mode()[0]
    print(f'MODE Trip Combo for {ride_count} rides: {popular_combo}')
...
def trip_duration_stats(df):
    ...
    print(f'Total Travel Time of {ride_count} rides: {total_duration} hrs')

    # Calculate and display mean travel time in minutes
    avg_travel =(df['Trip Duration'].mean()/60).round(2)
    print(f'MEAN travel time across {ride_count} rides: {avg_travel} min')
...
def user_stats(df):
    ...
    if 'Birth Year' in df.columns:
        ...
        min_age = int(df_filter_nan["Rider Age"].min())
        print(f'MIN Rider Age: {min_age}')

        mode_age = int(df_filter_nan['Rider Age'].mode().iloc[0])
        print(f"MODE Rider Age: {mode_age}")

        max_age = int(df_filter_nan['Rider Age'].max())
        print(f"MAX Rider Age: {max_age}")
...
def filter_by_month(df):
    ...
    month_filter = months_of_year[monthNum-1]
    print(f'--- Month Selected: {month_filter} ---')   
    return filtered_df
